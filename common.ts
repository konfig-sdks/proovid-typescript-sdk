...
import { readableStreamToString, ProoViDError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ProoViDError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ProoViDError(e, e.response?.data)
                }
                if (innerError instanceof ProoViDError) {
                    // Got "ProoViDError" from the above try block
                    throw innerError;
                }
